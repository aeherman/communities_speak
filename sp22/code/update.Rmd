...
final_clean %>%
  ...
  group_by(source, week) %>% count %>% arrange(week) %>%
  ggplot(aes(x = factor(week), y = n, group = source, color = source)) +
  geom_point() + geom_line(show.legend = FALSE) +
  ggrepel::geom_text_repel(aes(label = n), show.legend = FALSE, seed = 1) +
  xlab("\nActive Weeks of Survey") + ylab("New Responses\n") +
  ggtitle("New Responses Over Time\n") +
