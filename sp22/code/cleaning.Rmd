...
survey_id <- gs4_find() %>% filter(str_detect(name, "Individual Survey") & str_detect(name, "May")) %>% select(id, name)
...
final_clean <- final_clean %>%
  ...
  select(responseid, recordeddate, duration, userlanguage, source, order,
...
wrangled <- final_clean %>% #mutate(completion = rowSums(!is.na(final_clean))/ncol(final_clean)) %>%
  ...
  mutate(
    ...
    across(c(q34_4, q34_5, q34_8, q34_9), ~ . < 3, .names = "{col}_good"),
    across(c(q34_4, q34_5, q34_8, q34_9), ~ . > 3, .names = "{col}_bad"),
...
key34_ <- colnames(wrangled)[str_detect(colnames(wrangled), "34") & str_detect(colnames(wrangled), "good|bad")]
key34 <- str_replace(key34_, "_(good|bad)", "")
indeces34 <- which(survey_codebook_labelled$q %in% key34)
values34 <- survey_codebook_labelled$full_label[indeces34]
values34_ <- unlist(lapply(values34, function(value34) paste(value34, c("good", "bad"), sep = "_")))
names(wrangled)[match(key34_, names(wrangled))] <- values34_
