...
survey <- read_csv("../data/input/survey_20220408.csv") %>%
...
final_clean <- survey[c('responseid', 'recordeddate')] %>% left_join(clean_data()) %>%
  ...
  mutate(valid = case_when(
    completion > 0.5 & q2 == 2 & q5 > 17 & !is.na(borough) ~ 1,
    TRUE ~ 0
  )) %>%
  mutate(valid = ifelse(if_any(everything(), str_detect, pattern = "^test$"),
                        NA_integer_,
                        valid),
...
# 39
index_40 <- str_which(column_names$q, "q40")
key_40 <- column_names$q[index_40]
label_40 <- column_names$label[index_40]
q40_bi <- final_clean %>% select(responseid, source, contains("40")) %>%
  mutate_at(vars(contains("40")), ~!str_detect(haven::as_factor(.), "almost|never")) #%>% rename_at(vars(contains("41")), ~glue::glue("{.x}_bi"))
names(q40_bi)[match(key_40, names(q40_bi))] <- paste(label_40, "bi", sep = "_")
