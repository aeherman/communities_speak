...
wrangled <- final_clean %>% #mutate(completion = rowSums(!is.na(final_clean))/ncol(final_clean)) %>%
  ...
  mutate(
    ...
    across(c(q40_2, q40_3),
           ~labelled(5-.,5-attributes(final_clean$q40_3)$labels), .names = "stress_{col}"),
    across(c(q40_1, q40_4),
           ~labelled(.-1,attributes(final_clean$q40_4)$labels-1), .names = "stress_{col}"),
    ...
    hh_size = rowSums(across(starts_with("26"))),
...
indeces_40 <- which(str_detect(survey_codebook_labelled$q, "q40"))
key_40 <- paste0("stress_", survey_codebook_labelled$q[indeces_40])
value_40 <- paste0("stress_", survey_codebook_labelled$full_label[indeces_40])
names(wrangled)[match(key_40, names(wrangled))] <- value_40
```
