...
```{r include=F}
selected <- final_clean %>% filter(is.na(test_response)) %>%
  select(contains("q"), order) %>% select(!matches("text")) %>% filter(q2 == 1)
...
trimws(stringi::stri_trans_general(sub %>% pull(block_title) %>% .[2], "latin-ascii")
