...
cat("None of these plots show statistically significant results")
```
...
make_plots(wrangled %>% filter(inc_be_pov_before == 0), # filter for those who started above the poverty line
           ...
           title = "Respondents whose household income\ndropped below the poverty line", show=TRUE)
cat("Plots for hh_sn_65_bi, hh_ch_0_17_bi, borough do not have\n at least one statistically significant result")
```
...
cat("Plots for gen and hh_ch_0_17_bi do not show at least one statistically significant result")
```
...
cat("None of these plots show at least one statistically significant result")
```
...
c.	Somewhat food insecure = worried about food not lasting (OR experienced food bought didn’t last i.e. 1 indicator?)
...
make_plots(wrangled, fin_sec_dem, "food_insec",
           title = "Households that experienced food insecurity in the past year", show = TRUE)

cat("Plots for gen and hh_sn_65_bi do not have at least one statistically significant result.")
```
