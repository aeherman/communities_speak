...
make_plots(wrangled, "bronx_bi", "rate_neigh_pol_bad", title = "Negatively Rated Police Responsiveness", show = TRUE) #added by Brendan
```
...
make_plots(wrangled, "inc_dist", "rate_neigh_pre_good", title = "Rated Crime Prevention Poorly", show = TRUE)
```
...
wrangled %>% ggplot(aes(x = rate_gov_all_ave)) + geom_bar() + geom_vline(xintercept = mean_val) + ggtitle("Avg. rating for city, state, federal gov't", subtitle = "1=excellent -> 5=very poor") #title added by Brendan
