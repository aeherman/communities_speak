...
make_plots(wrangled, demographics, "emp_change")
```
...
make_plots(wrangled, demographics, "emp_after_un")
```
...
make_plots(wrangled %>% filter(emp_change == 1),
           ...
           title = "Respondents who experienced\nan adverse or neutral/positive job change")
```
...
make_plots(df = wrangled,
           ...
           title = "Proportion to Experience a job change")
...
make_plots(wrangled %>% mutate(temp = unemp_ben == 1), demographics, "temp")
```
...
lapply(emp_reas[1:2], function(reason) {
  make_plots(wrangled, demographics, reason, title = reason)
})
```