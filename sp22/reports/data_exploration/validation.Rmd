...
```{r}
final_clean %>%
  mutate(hh_size = rowSums(across(starts_with("q26")))) %>%
  filter(hh_size > 20) %>% select(q5, completion, q8, starts_with("q26"))

wrangled %>% filter(age >= 65) %>% filter(hh_sn_65_bi == 0) %>% select(age, starts_with("hh"))
wrangled %>% filter(age >= 18, age < 65, hh_ad_18_64 < 1) %>% select(age, mar, hh_ad_18_64)
wrangled %>% filter(mar == 2) %>% select(age, mar,hh_sn_65_bi, hh_ad_18_64)

hh <- c("responseid", "age", "hh_sn_65", "hh_ad_18_64", "hh_ch_4_17", "hh_ch_0_4", "hh_size")

range(wrangled$hh_size, na.rm = TRUE)
table(wrangled$hh_ad_18_64)
table(wrangled$hh_sn_65)
wrangled %>% count(any_else)
wrangled %>% filter(hh_size >= 20) %>% select(responseid, age, relig, borough, starts_with("hh") & !ends_with("bi")) %>% arrange(desc(hh_size)) %>% mutate(relig = labelled::to_factor(relig))


wrangled %>% filter(hh_size == 0)
wrangled %>% select(age, hh_sn_65, hh_ad_18_64, hh_ch_4_17, hh_ch_0_4)

wrangled %>% select(hh, mar) %>%
  filter(!if_all(contains("hh"), is.na)) %>%
  mutate(test = case_when(
    age > 65 & hh_sn_65 == 0 | age > 17 & age < 65 & hh_ad_18_64 == 0  ~ FALSE,
    TRUE ~ TRUE
  )) %>% filter(!test) %>% mutate(mar = labelled::to_factor(mar))


wrangled %>% filter(mar == 2, hh_sn_65 + hh_ad_18_64 < 2) %>% select(hh)
...
non_ny <- selected %>% filter(q2 == 2) %>% nrow # why didn't it get fixed in the cleaning script?
n <- selected %>% nrow
```
...
```{r}
by_reqs <- final_clean %>% filter(is.na(test_response)) %>% group_by(
  q2 == 1,
  completion >= 0.5,
  duration >= 100
) %>% count
...
```{r}
test <- final_clean %>% distinct %>% filter(is.na(test_response)) %>%
  group_by(order) %>% group_split()
sub <- survey_codebook %>% select(q, question, block_title) %>%
  mutate(
    block_title = trimws(stringi::stri_trans_general(block_title, "latin-ascii")),
    block = str_replace_all(block_title, " ", ""))

order <- test[[1]] %>% select(order) %>% distinct %>%
  tidytext::unnest_regex(output = block, input = order, pattern = "\\|") %>%
  mutate(order = row_number()) %>% bind_rows(tibble(block = c("newyorkresident?", "closing"),
       order = c(0, 7)))

test[[1]] %>% #rownames_to_column() %>%
  mutate_all(as.character) %>%
  pivot_longer(cols = -responseid, names_to = "q") %>% 
  pivot_wider(names_from = responseid) %>%
  left_join(sub %>% full_join(order)) %>% filter(!is.na(block_title)) %>% arrange(order) %>%
  filter(!str_detect(q,"text|[:digit:]{1,2}_[2-8]")) %>%
  mutate(q_order = row_number())

stringi::stri_trans_general(sub %>% pull(block_title) %>% .[2], "latin-ascii")
utf8::utf8_print(trimws(sub %>% pull(block_title) %>% .[2]), utf8 = FALSE)

sub %>% filter(!str_detect(q,"text|[:digit:]{1,2}_[2-8]"))

length(test)


length(test)
count(final_clean %>% filter(is.na(test_response)), order)

out <- lapply(test, function(element) {
  #for(element in test) {
  #i <- i + 1
  order <- element %>% select(order) %>% distinct %>%
    tidytext::unnest_regex(output = block, input = order, pattern = "\\|") %>%
    mutate(order = row_number()) %>%
    # add unrandomized blocks
    bind_rows(tibble(block = c("newyorkresident?", "closing"), order = c(0,7))) %>%
    full_join(sub, by = c("block")) %>% filter(!is.na(block_title)) %>% arrange(order, question) %>%
    filter(!str_detect(q, "text|[:digit:]{1,2}_[2-9]|_[[:digit:]]{2}"),
           !is.na(order))
  
  out <- element %>% mutate_all(as.character) %>%
    
    pivot_longer(cols = -responseid, names_to = "q") %>%
    pivot_wider(names_from = responseid) %>%
    right_join(order, by = c("q")) %>% arrange(order) %>%
    #filter(#!str_detect(q, "text|[:digit:]{1,2}_[2-9]|_[[:digit:]]{2}"),
    #       str_detect(q, "q")) %>%
    mutate(q_order = row_number()) %>%
    select(q_order, #q,
...
}) %>% reduce(full_join, by = c("q_order")) %>% arrange(q_order)

attrition <- out %>% mutate(q_order, rate = rowMeans(across(starts_with("r"), is.na)))

attrition %>% select(rate, starts_with("q"), q_order)
# 407
# 328
# 94

attrition %>% ggplot(aes(x = q_order, y = rate)) + geom_line() + geom_point()
...
```{r}
wrangled$order2 <- str_split(wrangled$order, pattern = "\\|")
wrangled[c("order", "order2")]
survey_codebook_labelled %>% select(q, question, block_title) %>%
  group_by(block_title) %>% summarize(min = min(question), max = max(question)) %>%
  mutate(range = glue::glue("q{min}, q{max}"))

selected

colnames(wrangled)
wrangled %>% select(order)

final_clean %>% filter(is.na(test), !is.na(order)) %>% select(responseid, order, starts_with("q"))
```
...
```{r}
poa %>% select(contains("mntl")) %>% select(!contains("bi")) %>%
  filter(if_all(everything(), ~floor(.) == mntl_hlth_unc)) %>%
  count(mntl_hlth_conf) %>% 
  ggplot(aes(x = reorder(to_character(mntl_hlth_conf), mntl_hlth_conf), y = n)) +
  geom_col(fill = project_pal[4]) +
  geom_text(aes(label = n), vjust = -0.2) +
  xlab(NULL) + ylab(NULL) +
  ggtitle("Respondents who only answered one category to all mental health questions")

poa %>% select(contains("rate")) %>%
  filter(if_all(everything(), ~.==rate_neigh_med)) %>%
  count(rate_neigh_med) %>%
  ggplot(aes(x = reorder(to_character(rate_neigh_med), rate_neigh_med), y = n)) +
  geom_col(fill = project_pal[4]) +
  geom_text(aes(label = n), vjust = -0.2) +
  xlab(NULL) + ylab(NULL) +
  ggtitle("Respondents who only answered one category to all rating questions")
