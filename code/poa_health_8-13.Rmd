...
make_plots(df, demographics, hyp_var = "mntl_hlth_bi", title = "Proportion of Respondents Experiencing\nat Least One Mental Health Challenge")
```
...
```{r warning=F, echo = F}
make_plots(df, by_vars = "unemployed", hyp_var = "mntl_hlth_bi", conf = .15)
...
```{r warning=F, echo = F}
make_plots(df %>% mutate(con_vrnts = vrnts == 1), by_vars = demographics, hyp_var = "con_vrnts",
