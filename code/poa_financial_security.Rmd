...
```{r}
categories <- attributes(poa$inc_after)$labels
...
arranged <- tibble(range = names(categories)) %>%
  left_join(poa %>% mutate(range = labelled::to_character(inc_before)) %>%
...
ggplot(data = poa, aes(x = inc_before)) + geom_bar(fill = project_pal[4])
```
...
```{r}
plots2.1 <- make_plots(df = poa, by_vars = demographics, hyp_var = "inc_neg", min = 8,
...
```{r}
(became_unemp <- poa %>% filter(str_detect(emp_status_after, "unemp")) %>%
...
plots2.1$emp_status_after +
  geom_col(data = tibble(emp_status_after = "Unemployed", prop = became_unemp$prop),
           fill = project_pal[3]) +
  geom_label(aes(x = .18, y = 3), label = glue::glue("{paste(became_unemp[c('n','n_n')],
                              collapse = '/')}\nPreviously\nEmployed"),
             color = project_pal[4], fill = project_pal[3])
```
```{r eval=F, include=F}
item <- "res_cat"
sym_item <- sym(item)
poa %>% group_by_at(vars(!!sym_item, race_census, weight)) %>%
...
```{r}
poa %>% count(inc_drop_pov, inc_dist) %>%
  mutate_if(is.labelled, labelled::to_character) %>% na.omit

make_plots(poa, by_vars = demographics, hyp_var = "inc_drop_pov",
           min = 7,
...
arranged
counted <-
  poa %>% filter(inc_dist == 3, inc_drop_pov == 1) %>% count(inc_before) %>% rename(dropped = n)
...
There were not enough respondents in this category to meaningfully analyze this hypothesis.  We would have needed at least 5 sucesses.

```{r echo=F}
poa %>% count(unemp_ben) %>% mutate_if(is.labelled, labelled::to_character)

poa %>% filter(unemp_ben == 1) %>% count(inc_neg)
```
...
```{r}
sum(poa$emp_before_un)
mean(poa$emp_before_un)

poa %>% count(emp_before_un, inc_neg) %>% mutate_if(is.labelled, labelled::to_character) %>% na.omit

make_plots(poa, by_vars = "emp_before_un", hyp_var = "inc_neg",
...
```{r}
poa %>% filter(part_time == 1) %>% count(emp_before)

make_plots(df = poa, by_vars = "part_time", hyp_var = "inc_neg", show = "yes")
```
...
```{r}
mean(poa$diff_bill, na.rm = TRUE)

make_plots(df = poa, demographics, "diff_bill", min = 10, title = "Proportion of Respondents who had difficulty\npaying bills in the past year")[-6]
```
...
```{r}
mean(poa$diff_rent, na.rm = TRUE)

make_plots(df = poa, demographics, "diff_rent", min = 10, title = "proportion of respondents who had difficulty\npaying rent in the past year")
```
...
```{r}
mean(poa$fin_unstable, na.rm = TRUE)

poa %>% filter(is.na(diff_ran_out), !is.na(diff_bill))

make_plots(poa, demographics, hyp_var = "fin_unstable", min = 10)
```
...
```{r}
mean(poa$food_insecurity, na.rm = TRUE)

make_plots(poa, demographics, "food_insecurity", min = 7)[-6]
```
...
```{r}
mean(poa$hh_ch_0_17_bi == 1 & poa$food_insecurity, na.rm = TRUE)

make_plots(poa, "hh_ch_0_17_bi", hyp_var = "food_insecurity")
```
...
```{r}
make_plots(poa %>% mutate(new_var = rate_gov_cit < 3),
           by_vars = "inc_neg", hyp_var = "new_var", show = "yes",
...
```{r}
mean(poa$exp_ab_or_vi, na.rm = TRUE)

make_plots(poa, by_vars = "inc_be_med_after", hyp_var = "exp_ab_or_vi")
```
...
```{r}
make_plots(poa %>% filter(hh_ch_0_17_bi == 1), "inc_be_med_after", "con_trans")
```

```{r include=F, eval=F}
drive_upload(media = "poa_financial_security.pdf", path = paste0("Communities Speak/Subteams/Data Subteam/Data Findings + VIZ_IS_2/Findings/POA_Financial_Security_FULL_ARIELLE_0404.pdf"))
```
