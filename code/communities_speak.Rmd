...
#for (col in colnames(survey)[-1]) {
    ...
    if(col %in% c(likert, simple)) {
      ...
      relabelled <- lapply(cols_to_label, function(dummy){
        ...
        out %>% ungroup %>% transmute_at(vars(dummy), ~labelled(as.integer(.), values))
      }) %>% reduce(bind_cols)
      
      out[cols_to_label] <- relabelled
        
        #r_zaiyctblk6t1z33
    } else if(all(!str_detect(survey[[col]], "[:alpha:]"), na.rm = TRUE) & any(!is.na(survey$q6_5_text))) {
      out <- survey[c("responseid", col)] %>% mutate_at(vars(col), as.integer)
    } else {
      out <- survey[c("responseid", col)]  
    }
...
#}    
    ...
    ###
    mutate(q5 = lapply(survey$q5, function(element){
...
wrangled <- final_clean %>% #mutate(completion = rowSums(!is.na(final_clean))/ncol(final_clean)) %>%
  ...
  mutate(
    ...
    race_black_his = str_detect(q7, "black|hispanic"),
    ## 6.3
...
write_dta(wrangled, path = paste0("../data/output/wrangled", today,".dta"), version = 15)
