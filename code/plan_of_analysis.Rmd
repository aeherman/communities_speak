...
```{r}
plots <- make_plots(df = filter(poa, !str_detect(emp_b, "unemp")),
           dems = dems_employment, var = "unemployed", min = 5)
length(filter(poa, !str_detect(emp_b, "unemp"), borough == "bronx")$unemployed == 1)
...
make_plots(df = poa %>% mutate(inc_neg = inc_neg == 1),
           ...
           title = "Proportion to Experience and Adverse job change")
```
