...
prepped <- wrangled %>% mutate_at(vars(borough), ~str_to_title(str_replace(., "si", "staten island"))) %>%
  mutate(zip = str_replace_all(zip, "-.*| and.*", ""))

dist <- prepped %>% count(borough) %>% na.omit() %>% mutate(prop = n/sum(n))
dist_zip <- prepped %>% count(zip) %>% mutate(prop = n/sum(n))
dist_child_zip <- prepped %>% filter(hh_ch_0_17_bi == 1) %>% count(zip) %>% mutate(prop = n/sum(n))
dist_child <- prepped %>% filter(hh_ch_0_17_bi == 1) %>% count(borough) %>% na.omit %>% mutate(prop = n/sum(n))

# make sure to check on NA values!
mean(dist_zip$n == 1)
...
by_zip <- nyc_zip %>%
  ...
  scale_fill_continuous(glue::glue("N = {nrow(wrangled)}"),
                        high = "#132B43", low = "#56B1F7", labels = scales::percent) +
  ...
  ggrepel::geom_label_repel(
    data = filter(nyc_zip, ZIPCODE == dist_zip$zip[which.max(dist_zip$prop)]),
...
by_zip_child <- 
  ...
  ggplot() + geom_sf(aes(fill = prop), color = project_pal[1]) +
  scale_fill_continuous(glue::glue("N = {nrow(filter(wrangled, hh_ch_0_17_bi == 1))}"),
                        high = "#132B43", low = "#56B1F7", labels = scales::percent) +
  guides(fill = guide_legend(title.position = "bottom")) +
  labs(
    title = "Respondents with Children by Zip Code",
    #Source: https://data.cityofnewyork.us/widgets/i8iw-xf4u\n
    #subtitle = glue::glue("Respondents: {nrow(filter(wrangled, hh_ch_0_17_bi == 1))}\n{sf::st_crs(nyc_zip)$input}\n\n")
    ) +
  ggrepel::geom_label_repel(
    data = right_join(nyc_zip, dist_child_zip[which.max(dist_child_zip$prop), ],
                      by = c("ZIPCODE" = "zip")),
    aes(label = glue::glue("Zip {ZIPCODE}: {scales::percent(prop)}"), geometry = geometry),
    stat = "sf_coordinates", #force_pull = 10, force = 10,
    min.segment.length = 0, seed = 1, nudge_x = -0.05, nudge_y = 0.1, fill = project_pal[1]) +
  xlab(NULL) + ylab(NULL)
...
by_bor_child <- nybb %>%
  #sf::st_transform(crs = 4326) %>%
  left_join(dist_child, by = c("boro_name" = "borough")) %>%
  ...
  ggrepel::geom_label_repel(aes(label = glue::glue("{boro_name}: {scales::percent(prop)}"),
                            ...
                            fill = project_pal[1]) +
  xlab(NULL) + ylab(NULL) +
  labs(title = "Respondents with Children by Borough",
       #subtitle = glue::glue("Respondents: {nrow(filter(wrangled, hh_ch_0_17_bi == 1))}\nCRS: {sf::st_crs(nybb)$input}\n\n")
       )
...
```{r}
prepped$borough
prepped$race_weight[is.na(prepped$weight)]

prepped_cc <- prepped %>% filter(hh_ch_0_17_bi == 1, borough != "Staten Island", !is.na(find_cc))

all <- tibble(borough = "All", n = sum(prepped_cc$hh_ch_0_17_bi, na.rm = TRUE), prop = 1-mean(prepped_cc$find_cc, na.rm = TRUE))
all_weighted <- tibble(borough = "All", n = sum(prepped_cc$hh_ch_0_17_bi, na.rm = TRUE),
                       prop = 1-mean(prepped_cc$find_cc*prepped_cc$weight, na.rm = TRUE))

p_find_cc <- prepped_cc %>% filter(!is.na(find_cc)) %>%
  mutate(find_cc_weighted = find_cc*weight) %>%
  group_by(borough, find_cc) %>%
  # weighted is similar to unweighted
  #summarize(n = ifelse(find_cc == 0, n(), sum(find_cc_weighted, na.rm = TRUE))) %>% distinct() %>% 
  count(borough, find_cc) %>%
  group_by(borough) %>% mutate(prop = n/sum(n, na.rm = TRUE)) %>%
  filter(find_cc == 0) %>% select(-find_cc) %>%
  # which total?
  rbind(all) %>%
  ggplot(aes(x = borough, y = prop, fill = borough != "All")) +
  geom_col(show.legend = FALSE) +
  geom_text(aes(label = scales::percent(prop)), vjust = -.3, inherit.aes = TRUE) +
  scale_fill_manual(values = project_pal[c(3,4)]) +
  scale_y_continuous(labels = scales::percent, limits = c(0,1)) + xlab(NULL) + ylab(NULL) +
  ggtitle("Percent Respondents with Children who Needed Childcare\nbut Couldn't Find it")
```

```{r include=F}
ggsave(by_bor, filename = "respondents_by_borough.png")
ggsave(by_zip, filename = "respondents_by_zip.png")
ggsave(by_zip_child, width = 5.5, height = 5, filename = "respondents_child_by_zip.png")
ggsave(by_bor_child, width = 5.5, height = 5, filename = "respondents_child_by_bor.png")
ggsave(p_find_cc_weighted, height = 5, filename = "p_find_cc_weighted.png")
ggsave(p_find_cc, height = 5, filename = "p_find_cc.png")
